  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-white/60">Browse {consoles.length} audio consoles</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {consoles.map((console) => (
          <Link key={console.id} href={`/equipment/consoles/${console.id}`}>
            <Card className="h-full bg-gradient-to-br from-[#0B0C1A]/80 to-[#121227]/80 backdrop-blur-xl border-white/10 hover:border-[#00E8FF]/30 transition-all cursor-pointer">
              <CardHeader>
                ...
                <p className="text-white font-medium">{console.model}</p>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-white/60">Type</span>
                  <Badge variant="secondary">{console.type}</Badge>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-white/60">Channels</span>
                  <span className="text-white">{console.channels}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-white/60">Faders</span>
                  <span className="text-white">{console.faders}</span>
                </div>
                {console.applications && console.applications.length > 0 && (
                  <div className="pt-2">
                    <div className="flex flex-wrap gap-1">
                      {console.applications.slice(0, 3).map((app, index) => (
                        <Badge
                          key={index}
                          variant="outline"
      ...
      {consoles.length === 0 && (
        <div className="text-center py-12">
          <p className="text-white/60">No consoles found matching your criteria</p>
        </div>
      )}
    </div>
  );
